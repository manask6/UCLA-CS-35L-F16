...
void * split_image(void* threadNum)
{
  ...
  for( int px = *(int*)threadNum; px<width; px += nthreads )
	...
	for( int py=0; py<height; ++py )
	{
	   ...
	   */
	  WHCarray[px][py][0] = scaled_color[0];
	  WHCarray[px][py][1] = scaled_color[1];
	  WHCarray[px][py][2] = scaled_color[2];
	}
  }
  return NULL;
}
...
/* Write the image format header */
  ...
  {
  	thread_id[t] = t;
	int r = pthread_create(&threads[t], NULL, split_image,&thread_id[t]);
	if(r)
	{
	  fprintf(stderr, "Thread error",r);
	  exit(-1);
	}
  }
  for(int t = 0; t < nthreads; t++)
  {
	int r = pthread_join(threads[t], NULL);
	if(r)
	{
	  fprintf(stderr, "Thread error", r);
	  exit(-1);
  ...
  for(int i = 0; i < width; i++)
  {
  	for(int j = 0; j < height; j++)
  	  	printf("%.0f %.0f %.0f\n", WHCarray[i][j][0], WHCarray[i][j][1], WHCarray[i][j][2]);
